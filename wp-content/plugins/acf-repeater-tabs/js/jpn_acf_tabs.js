...
jQuery(document).ready(function ($) {
    ...
    if ($('.jpn-tabs-activated > .acf-repeater').length > 0) {
        ...
        // Main function
        function jpn_acf_tabs(add = false, newrow = false, parent = false, copy = false) {
            ...
            $('.jpn-tabs-activated').each(function() {
                ...
                $('[data-jpn="'+jpn+'"] > .acf-repeater > table > tbody > tr.acf-row:not(.acf-clone)').each(function() {
                    ...
                    if (newrow == 'last' && jpn == add) {
                        ...
                        sectionHeadingPlaceholder = 'Item ' + activeNum;
                    ...
                    $('[data-jpn="'+jpn+'"] > .jpn-acf-tabs > .nav > ul').append('<li data-jpn-index="'+index+'" data-jpn-tab-div="'+tabID+'" class="jpn-acf-tab-div'+css+'"><div class="jpn-tab-hover"><a href="#" class="jpn-tab-copy" data-jpn-tab-id="'+tabID+'" data-jpn-num="'+index+'"><span class="hide-h">COPY </span><span class="dashicons dashicons-clipboard"></span></a></div><a href="'+tabID+'" class="jpn-acf-tab'+css+' '+collapsed+'" data-jpn-nav="'+jpn+'" data-jpn-tab="'+tabID+'"><span>'+ (($('[data-jpn-tab-id="'+tabID+'"] [data-name="section_heading"] input').val()) ? $('[data-jpn-tab-id="'+tabID+'"] [data-name="section_heading"] input').val() : num) +'</span></a></li>');

                    /* I wanted to add a default of "Item (tabID)" to the tab labels. This does that, but is buggy as hell  
                    * $('.repeater-dw .tabbed-repeater-top.jpn-tabs-activated > .jpn-acf-tabs > .nav > ul > li.active > .jpn-acf-tab > span').prepend(sectionHeadingPlaceholder);
                    */


                    /** 
                    * I don't remember commenting this out. It may have been commented out in the original source.
                    **
        ...
        //On Page Load set the height of the container to avoid things jumping around when a new row is added
        $(function(){
            $('body').addClass('bpl-repeater-page-body');
        });
        ...
        $(document).on('click', '.jpn-tabs-activated a[data-event="add-row"].jpn-tab-button', function(e) {
            ...
            if($('.tabbed-repeater').first().height() < $('.jpn-acf-tabs').first().height()){
                $('.tabbed-repeater').first().height($('.jpn-acf-tabs').first().height() + 40);   
            }
        ...
        $(document).on('click','.jpn-acf-tab', function(e) {
            e.preventDefault();
            if(!$(this).closest('.acf-field-repeater').find($('.jpn-acf-tab.active span')).first().html().length > 0) {
                $(this).closest('.acf-field-repeater').find($('.jpn-acf-tab.active span')).first().html('Item ' + $(this).attr('data-jpn-index'));
            }
            //$('.jpn-acf-tab-div.active span').html($('.acf-row.active [data-name="section_heading"] input').val());
            //I Can't get the tabs to stay the same width when creating a new tab
            //$('.jpn-acf-tab-div.active > a').css('width', $('.jpn-acf-tab-div.active > a span').outerWidth() + 15 + 'px');
        ...
        // On Label Change...
        $(document).on('keyup','.acf-row.active [data-name="section_heading"] input', function(e) {
            if($(this).closest('.tabbed-repeater').find($('.nav a.jpn-acf-tab.active span')).first().html().length > 0) {
                $(this).closest('.tabbed-repeater').find($('.nav a.jpn-acf-tab.active span')).first().html($(this).val());
            } else {
                $(this).closest('.tabbed-repeater').find($('.nav a.jpn-acf-tab.active span')).first().html($(this).val());
            }
        });
        ...
        //No clue what this is supposed to do. Maybe it was supposed to navigate between your tabs with the scroll wheel?
        ...
        $(document).on('mousedown', 'a.acf-confirm-y', function(e) {
            if (jpn_confirm_delete) {
                ...
                setTimeout(function(){ jpn_acf_tabs(jpn_confirm_delete, false, parent, 'copy'); }, 650);
            }
        });

        //Clicks the hidden wordpress remove button and moves the confirmation tooltip to the proper location.
        $(document).on('click', 'a.jpn-remove-row', function(e) {
            ...
            if($('.tabbed-repeater').length > 0 ){
                $('.acf-tooltip.top.-confirm').css({top: $(this).closest('a.jpn-remove-row').offset().top - 30, left: $(this).closest('a.jpn-remove-row').offset().left - 65});
            }
        });
        ...
        /**
        * Not sure what this does. Disabling it didn't seem to break anything.
        **
        ...
        //Reorder posts in DB when title is drag n' dropped
        ...
        acf.addAction('load', function($el){jpn_acf_tabs();});
    }
    //Move SEO Ultimate meta box to bottom of page
    $(function(){
        function moveSeoUltMetaBox(){
            $('#su_postmeta').appendTo('#postbox-container-2');
        }
        moveSeoUltMetaBox();
    });
